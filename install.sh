...
if [ ! -f "$TASK_DB" ]; then
    if [ -f "./app/db/data.db.sql" ]; then
        echo "   → Создаём data.db из файла схемы..."
        sqlite3 "$TASK_DB" < "./app/db/data.db.sql"
    else
        echo "❌ Ошибка: Файл ./app/db/data.db.sql не найден!"
        exit 1
    fi
else
...
if [ ! -f "$USERS_DB" ]; then
    if [ -f "./app/db/photo.db.sql" ]; then
        echo "   → Создаём photo.db из файла схемы..."
        sqlite3 "$USERS_DB" < "./app/db/photo.db.sql"
    else
        echo "❌ Ошибка: Файл ./app/db/photo.db.sql не найден!"
        exit 1
    fi
else
